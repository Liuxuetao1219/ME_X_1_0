...
def rope_apply(x: torch.Tensor, grid_sizes: torch.Tensor, freqs: torch.Tensor) -> torch.Tensor:
    ...
    def _make_freq_grid(f: int, h: int, w: int):
            ...
            fpart[:f].view(f, 1, 1, -1).expand(f, h, w, -1),   # [f,h,w,c_f]
            hpart[:h].view(1, h, 1, -1).expand(f, h, w, -1),   # [f,h,w,c_h]
            wpart[:w].view(1, 1, w, -1).expand(f, h, w, -1),   # [f,h,w,c_w]
        ], dim=-1).reshape(f*h*w, 1, -1)          # [seq_len,1,C]
    ...
    for g_idx, (f, h, w) in enumerate(uniq.tolist()):
        ...
        freq_grid = _make_freq_grid(f, h, w)  # [seq_len,1,C]
...
class WanSelfAttention(nn.Module):
    ...
    def forward(self, x, seq_lens, grid_sizes, freqs,
                ...
                tactile_q: torch.Tensor = None,
                tactile_k: torch.Tensor = None,
                tactile_v: torch.Tensor = None,
        ...
        Args:
            ...
            action_q/k/v(Tensor, optional): Action expert Q/K/V
            tactile_q/k/v(Tensor, optional): Tactile expert Q/K/V
        """
        ...
        if action_q is not None or tactile_q is not None:
            ...
            if tactile_q is not None:
                q_parts.append(tactile_q)
                k_parts.append(tactile_k)
                v_parts.append(tactile_v)
                L_tactile = tactile_q.size(1)
            else:
                L_tactile = 0
            ...
            attn_out = flash_attention(
                ...
                k=k_cat,
                v=v_cat,
                k_lens=None if attn_mask is not None else seq_lens,
                attn_mask=attn_mask,
            ...
            if tactile_q is not None:
                tactile_out = attn_out[:, start_idx:start_idx+L_tactile, :, :]
                outputs.append(tactile_out)
            else:
                outputs.append(None)
